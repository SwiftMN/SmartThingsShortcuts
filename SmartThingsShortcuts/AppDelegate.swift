...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([UIUserActivityRestoring]?) -> Void) -> Bool {
        ...
        guard let intent = userActivity.interaction?.intent as? ExecuteSceneIntent else {
            💩("Could not handle unknown intent")
            return false
        }

        guard let sceneId = intent.sceneId else {
            💥("Could not handle intent with invalid scene")
            return false
        }
